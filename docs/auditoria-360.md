...
|----|------|----------|-----------|---------|------------|------|---------|
| A1 ✅ | Dados/Produto | ~~Custo de origem não sobrevive à conversão proposta→obra~~ **fechado em 12/ago/2026** (`custo_origem` + `v_margem_obra`) | `docs/analise-catalogo.md` §3.1–3.3; `insumos_projeto.preco_unitario_base` grava preço de venda | Decisão de margem sobre número que a fonte não deu; margem de obra inexistente | Conversão colapsa base+ajuste num número e descarta o custo | **P1** | Guardar `custo_origem` separado do preço de venda em `insumos_projeto` |
...
| A7 ✅ | Regra de negócio | ~~Alterar `percentual_medido` pós-aprovação~~ **já estava fechado** por `fn_medicao_campos_imutaveis` (10/ago), um dia antes desta auditoria | `docs/analise-financeiro.md` §2.2 residual | % exibido desalinha do razão | Fan-out só roda na transição p/ Aprovada | **P3** | Recalcular no UPDATE de percentual ou bloquear edição pós-aprovação |
...
| A13 ✅ | Processo | ~~Fluxo ponta a ponta parcial~~ **fechado em 12/ago/2026** — `supabase/tests/fluxo_ponta_a_ponta.sql`, 10 asserções, transação revertida | §E | Coerência número-a-número entre todas as telas não exercitada em cada passo | Sem obra de teste com ciclo completo montada nesta sessão | **P2‑processo** | Montar uma obra `[AUDITORIA]` e percorrer proposta→faturamento (roteiro em §E) |

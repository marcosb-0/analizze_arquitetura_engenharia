-- Security fix: `fn_current_role() not in ('admin','gestao')` evaluates to NULL
-- (not TRUE) when the caller has no matching profiles row — anon, or any
-- authenticated user without a profile — because `NULL NOT IN (...)` is NULL
-- in SQL, and `IF NULL THEN` is false in PL/pgSQL. The guard silently let
-- those callers fall through to the rest of the SECURITY DEFINER function,
-- which bypasses RLS entirely. coalesce() makes the NULL case explicitly
-- deny instead of silently pass.
...
-- Defense in depth: Supabase's default privileges on the public schema grant
-- EXECUTE to anon/authenticated at CREATE time regardless of an explicit
-- `revoke ... from public` — revoke from anon by name too.
revoke execute on function public.fn_criar_projeto_padrao(uuid) from anon;
revoke execute on function public.fn_criar_projeto_padrao(uuid) from public;
